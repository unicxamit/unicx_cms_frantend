...
const CUSTOM_JS_GUARD_KEY = "__unicxCustomJsInitialized";
const CUSTOM_JS_PROMISE_KEY = "__unicxCustomJsInitPromise";

export function ensureCustomJsInitialized() {
  if (typeof window === "undefined") return Promise.resolve();

  if (window[CUSTOM_JS_GUARD_KEY]) return Promise.resolve();
  if (window[CUSTOM_JS_PROMISE_KEY]) return window[CUSTOM_JS_PROMISE_KEY];

  window[CUSTOM_JS_PROMISE_KEY] = (async () => {
    const hasCustomScriptTag = Array.from(document.scripts || []).some(
      (script) => typeof script?.src === "string" && script.src.includes("/assets/js/custom.js")
    );

    if (!hasCustomScriptTag) {
      await loadScript("js/custom.js");
    }

    window[CUSTOM_JS_GUARD_KEY] = true;
  })()
    .catch((error) => {
      delete window[CUSTOM_JS_PROMISE_KEY];
      throw error;
    });

  return window[CUSTOM_JS_PROMISE_KEY];
}
