...
import { getSubSubCategories, updateIndemandStatus } from "../adminApi";
import Loader from "../app/common/loader";
import "./serviceStyle/casestudy.css";

const IndemandService = () => {
  ...
  const fetchCategories = async () => {
    ...
    try {
      ...
      console.log("fetch indemand service error", err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleToggleStatus = async (id) => {
    setCategories((prev) =>
      prev.map((service) =>
        service?._id === id
          ? {
              ...service,
              Insrstatus: service?.Insrstatus === "active" ? "inactive" : "active",
            }
          : service,
      ),
    );
    ...
    try {
      const service = categories.find((f) => f?._id === id);
      const newStatus = service?.Insrstatus === "active" ? "inactive" : "active";
      await updateIndemandStatus(id, newStatus);
    } catch {
      fetchCategories();
    }
  };
  ...
  const filteredBlogs = categories.filter((blog) => {
    const searchMatch =
      ...
      blog?.indeman_service_name?.toLowerCase().includes(search);

    const statusMatch =
      selectedStatuses.length === 0 || selectedStatuses.includes(blog?.Insrstatus);
    const serviceMatch =
  ...
  return (
    <div className="category-container casestudy-page">
      <div className="casestudy-shell">
        <h3 className="heading_category casestudy-heading">Indemand Service</h3>
        ...
        <div className="casestudy-table-wrap">
          <table className="table table-bordered table-striped custom-table mt-4 casestudy-table indemand-table">
            <thead className="table-primary">
              <tr>
                <th className="col-id">ID</th>
                <th>Service Name</th>
                <th>Indemand Service</th>
                <th>Image</th>
              ...
              {loading ? (
                ...
                paginatedSubCategories.map((cat, index) => (
                  <tr key={cat._id}>
                    ...
                    <td data-label="Service Name">{cat?.name || "-"}</td>
                    <td data-label="Indemand Service">{cat?.indeman_service_name || "-"}</td>

                    <td data-label="Image">
                      {cat?.indeman_sericons ? (
                        <img
                          src={cat.indeman_sericons}
                          alt={`indemand service ${index + 1}`}
        ...
        <div className="casestudy-pagination-footer mt-4">
          ...
          <div className="casestudy-page-size">
            <label htmlFor="indemand-page-size">Rows per page</label>
            <select
              id="indemand-page-size"
...
export default IndemandService;
